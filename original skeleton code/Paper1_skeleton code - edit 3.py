...
#Add an option to compress a file with RLE
...
def SaveImage(Grid, Header):
  ...
  DoCompress=input('Do you want to compress? [Y]/[N]\n')
  if DoCompress=='Y':
      CompressImage(Grid,Header)
  else:
...
def CompressImage(Grid, Header):
    ...
    for Row in range (Header.Height):
        ...
        temp=''
        for Column in range(Header.Width):
            if Grid[Row][Column] != prechar:
                if prechar:
                    temp=temp+str(counter)+prechar
                counter=1
                prechar=Grid[Row][Column]
            else:
                counter+=1
        temp+=str(counter)+prechar
        FileOut.write(temp)
        FileOut.write('\n')
